...
@SpringBootTest /* 실제 서비스처럼 테스트 진행. Bean을 모두 객체 */
class TokenProviderTest {
    ...
    void generateToken() {
        ...
        /* 토큰에서 뽑은 값과 디비 값이 같을까? */
        assertThat(tokenId).isEqualTo(testUser.getId());
    }

    @DisplayName("토큰 만료 검사하기 -> 만료되면 유효성 검증 실패")
    @Test
    void validToken_expireAt() {
        /* given - 5일전 생성. */
        String token = JwtFactory.builder()
                .expiresAt(new Date(new Date().getTime() - Duration.ofDays(5).toMillis()))
                .build()
    ...
    @DisplayName("유효한 정상 토큰 생성 및 유효성 검사 ture")
    @Test
